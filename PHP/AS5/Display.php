...
$Movies = array( 
  ...
  for ($i=0; $i<count($Movies); $i++) {
    $Movie = new Movie($i);
    $Movie->set_Name($Movies[$i][0]);
    $Movie->set_Picture($Movies[$i][1]);
    $Movie->set_Director($Movies[$i][2]);
    $Movie->set_MainActor($Movies[$i][3]);
    $Movie->set_Link($Movies[$i][4]);
    $Movie->set_Year($Movies[$i][5]);
    $Movie->set_Genera($Movies[$i][6]);
    array_push($DisplayMovies, $Movie);
  }
  ...
  $root = $dom->createElement("Movies");
    ...
    foreach ($DisplayMovies as $i){
      $movie_node = $dom->createElement("Movie");
      $Title = $dom->createElement("Title", $i->get_Name());
      $movie_node->appendChild($Title);

      $Picture = $dom->createElement("Picture", $i->get_Picture());
      $movie_node->appendChild($Picture);

      $Director = $dom->createElement("Director", $i->get_Director());
      $movie_node->appendChild($Director);

      $MainActor = $dom->createElement("MainActor", $i->get_MainActor());
      $movie_node->appendChild($MainActor);

      $Link = $dom->createElement("Link", $i->get_Link());
      $movie_node->appendChild($Link);

      $Year = $dom->createElement("Year", $i->get_Year());
      $movie_node->appendChild($Year);

      $Genre = $dom->createElement("Genre", $i->get_Genera());
